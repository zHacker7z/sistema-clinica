  ...
  <div class="row" style="align-items: flex-start">
      ...
      <div class="card">
        ...
        <div class="row">
          ...
          <div class="field">
            ...
            <select v-model="timeHHMM">
              <option v-if="slots.length === 0" value="">
                Sem slots para esta data
              </option>
              <option
                v-for="s in slots"
                :key="s.timeHHMM"
        ...
        <div class="row" style="margin-top: 10px">
          <div class="field" style="min-width: 480px">
            ...
            <textarea
              v-model="notes"
              placeholder="Ex.: detalhes do atendimento"
            ></textarea>
          </div>
        </div>
        ...
        >
          ...
          <button
            :disabled="!selectedRescheduleId || submitting"
            @click="handleRescheduleSelected"
          >
            <CalendarSync
              :size="15"
              style="vertical-align: text-bottom; margin-right: 6px"
        ...
        <div v-if="weatherPreview" class="muted" style="margin-top: 8px">
          ...
          <strong>
            {{
              weatherPreview.status === "rain"
                ? "Chuva"
                : weatherPreview.status === "clear"
                  ? "Limpo"
                  : "Indisponível"
            }}
          </strong>
          <span
            v-if="
              typeof weatherPreview.precipitationProbabilityMax === 'number'
            "
          >
    ...
    <div style="flex: 1 1 360px">
      <div class="card">
        ...
        <div v-else-if="appointments.length === 0" class="muted">
        ...
        <div v-for="a in appointments" :key="a.id" style="padding-top: 10px">
          ...
          >
            <div>
              ...
              <div style="margin-top: 8px" v-if="a.weatherSnapshot">
                ...
                <span v-else class="badge" title="Previsão indisponível"
                  >Clima?</span
                >
              </div>
            </div>
            ...
            >
              ...
              <button class="danger" @click="handleDelete(a.id)">
                <Trash2
                  :size="15"
                  style="vertical-align: text-bottom; margin-right: 6px"
                />
              ...
              <select
                :value="a.status"
                @change="handleStatusChange(a.id, $event.target.value)"
              >
                <option value="scheduled">Agendado</option>
                <option value="completed">Concluído</option>
                <option value="cancelled">Cancelado</option>
          ...
          <hr
            style="border: none; border-top: 1px solid #eee; margin: 12px 0"
          />
        </div>
...
async function loadAppointments() {
  ...
  try {
    ...
    notify(
      extractErrorMessage(e, "Não foi possível carregar os agendamentos."),
    );
  } finally {
    loading.value = false;
  }
...
async function loadSlots() {
  try {
    ...
    if (
      !timeHHMM.value ||
      !slots.value.some((s) => s.timeHHMM === timeHHMM.value && s.available)
    ) {
...
async function handleCepLookup() {
  ...
  try {
    const { data } = await api.get(
      `/api/address/cep/${encodeURIComponent(value)}`,
    );
...
async function handleCreate() {
  ...
  try {
    ...
    if (status === "rain")
      notify("A previsão indica chuva no dia do atendimento.");
    else if (status === "clear")
      notify("Previsão: dia limpo (sem chuva esperada).");
    ...
    if (e?.response?.status === 409)
      notify("Horário indisponível: já existe outro agendamento.");
  } finally {
...
async function handleDelete(id) {
  if (
    !confirm("Tem certeza que deseja excluir este agendamento permanentemente?")
  )
    return;
  try {
    ...
    notify(extractErrorMessage(e, "Não foi possível excluir o agendamento."));
  }
}
...
function prefillReschedule(a) {
  ...
  notify(
    'Dados carregados. Ajuste data/slot e clique em "Remarcar selecionado".',
  );
}
...
async function handleRescheduleSelected() {
  ...
  try {
    await api.patch(
      `/api/admin/appointments/${selectedRescheduleId.value}/reschedule`,
      {
        dateISO: dateISO.value,
        timeHHMM: timeHHMM.value,
