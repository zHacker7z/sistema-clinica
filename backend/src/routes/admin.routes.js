  ...
  async (req, res, next) => {
    try {
      ...
      if (
        patientIdFilter &&
        patientIdFilter !== "all" &&
        mongoose.Types.ObjectId.isValid(patientIdFilter)
      ) {
  ...
  async (req, res, next) => {
    try {
      ...
      if (!appointment)
        return res
          .status(404)
          .json({ ok: false, error: "Agendamento não encontrado" });
  ...
  async (req, res, next) => {
    try {
      ...
      if (!isSlotTime(timeHHMM)) {
        return res
          .status(400)
          .json({ ok: false, error: "Use slots entre 08:00 e 17:00" });
      }
      if (isDateInPast(dateISO)) {
        return res
          ...
          .json({
            ok: false,
            error: "Não é permitido remarcar em data passada",
          });
      }
      ...
      const endAt = new Date(
        startAt.getTime() + appointment.durationMinutes * 60 * 1000,
      );
      if (isDateToday(dateISO) && isStartInPast(startAt)) {
        return res
          ...
          .json({
            ok: false,
            error: "Não é permitido remarcar para horário já passado",
          });
      }
      ...
      if (conflict) {
        return res
          .status(409)
          .json({ ok: false, error: "Conflito de horário na remarcação" });
      }
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      return res.json({ ok: true, itemId: id });
    } catch (err) {
      next(err);
    }
  },
